...
function chat_update_instance($chat) {
    ...
    if ($event->id = $DB->get_field('event', 'id',
        ['modulename' => 'chat', 'instance' => $chat->id, 'eventtype' => CHAT_EVENT_TYPE_CHATTIME])) {
...
function chat_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    $timeout = get_config("chat", "old_ping") * 10;
    ...
    if ($current) {
        ...
        $timeold = time() - get_config("chat", "old_ping");
        $timeold = floor($timeold / 10) * 10;  // Better db caching.
        $timeoldext = time() - (get_config("chat", "old_ping") * 10); // JSless gui_basic needs much longer timeouts.
...
/**
 * Function chat_get_latest_message
 *
 * @param $chatid
 * @param int $groupid
 *
...
function chat_login_user($chatid, $version, $groupid, $course) {
    ...
    if (($version != 'sockets') && $chatuser = $DB->get_record('chat_users',
            ['chatid' => $chatid, 'userid' => $USER->id, 'groupid' => $groupid])) {
        ...
        if (($chatuser->course != $course->id) || ($chatuser->userid != $USER->id)) {
            return false;
        }
...
function chat_delete_old_users() {
    ...
    $timeold = time() - get_config("chat", "old_ping");
    $timeoldext = time() - (get_config("chat", "old_ping") * 100);
...
function chat_calculate_next_chat_time(int $schedule, int $chattime): int {
    ...
    switch ($schedule) {
        case CHAT_SCHEDULE_DAILY:
            while ($chattime <= $timenow) {
                $chattime += DAYSECS;
            }
            break;
        case CHAT_SCHEDULE_WEEKLY:
            while ($chattime <= $timenow) {
                $chattime += WEEKSECS;
            }
            break;
...
/**
 * Function chat_format_message_manually
 *
 * @param $message
 * @param $courseid
 * @param $sender
 * @param $currentuser
 * @param null $chatlastrow
 *
...
function chat_format_message_manually($message, $courseid, $sender, $currentuser, $chatlastrow = null) {
    ...
    if (!empty($message->issystem)) {
        ...
        $output->basic = '<tr class="r1">
                            ...
                            <td class="cell c2 text">' .
            get_string('message' . $message->message, 'chat', fullname($sender)) . '</td>
                            <td class="cell c3">' . $message->strtime . '</td>
                          </tr>';
        if ($message->message == 'exit' || $message->message == 'enter') {
 ...
 * @throws dml_exception
 */
function chat_format_message_theme($message, $chatuser, $currentuser, $groupingid) {
    global $CFG, $OUTPUT, $COURSE, $DB;
    require_once($CFG->dirroot . '/mod/chat/locallib.php');

    static $users;     // Cache user lookups.

    if (file_exists($CFG->dirroot . '/mod/chat/gui_ajax/theme/config.php')) {
        include($CFG->dirroot . '/mod/chat/gui_ajax/theme/config.php');
    }

    if (isset($users[$message->userid])) {
        $sender = $users[$message->userid];
    } else if ($sender = $DB->get_record('user', ['id' => $message->userid],
    ...
    if (!empty($message->issystem)) {
        $message->message = get_string("message{$message->message}", 'chat', fullname($sender));
    }
...
/**
 * Function chat_extend_navigation
 *
...
function chat_extend_navigation($navigation, $course, $module, $cm) {
    ...
    if (has_capability('mod/chat:chat', context_module::instance($cm->id))) {
        ...
        $url = new moodle_url($target . 'gui_' . get_config("chat", "method") . '/index.php', $params);
...
function chat_view($chat, $course, $cm, $context) {
    ...
    $params = ['context' => $context, 'objectid' => $chat->id];
 ...
 *
 * @param  array $messages list of messages from a chat. It is assumed that these are sorted by timestamp in DESCENDING
 *                         order.
